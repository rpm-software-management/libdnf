...
int RepoCB::progress(double /*totalToDownload*/, double /*downloaded*/) {
    return 0;
}
void RepoCB::fastest_mirror(FastestMirrorStage /*stage*/, const char * /*ptr*/) {}
int RepoCB::handle_mirror_failure(const char * /*msg*/, const char * /*url*/, const char * /*metadata*/) {
    return 0;
}
...
const std::string & Repo::Impl::get_metadata_path(const std::string & metadata_type) const {
    ...
    if (ret.empty()) {
        logger->debug(tfm::format("not found \"%s\" for: %s", metadata_type, conf->name().get_value()));
    }
    return ret;
}
...
int Repo::verify_id(const std::string & id) {
    auto idx = id.find_first_not_of(REPOID_CHARS);
    return idx == id.npos ? -1 : static_cast<int>(idx);
}
...
void Repo::verify() const {
    if (p_impl->conf->baseurl().empty() &&
        ...
        throw std::runtime_error(tfm::format(_("Repository %s has no mirror or baseurl set."), p_impl->id));
    ...
    if (!type.empty()) {
        ...
        throw std::runtime_error(
            tfm::format(_("Repository '%s' has unsupported type: 'type=%s', skipping."), p_impl->id, type));
    }
}
...
bool Repo::load_cache(bool throw_except) {
    return p_impl->load_cache(throw_except);
}
...
std::string Repo::get_comps_fn() {
    auto tmp = p_impl->get_metadata_path(MD_TYPE_GROUP_GZ);
    if (tmp.empty())
        tmp = p_impl->get_metadata_path(MD_TYPE_GROUP);
    return tmp;
}


#ifdef MODULEMD
std::string Repo::get_modules_fn() {
    return p_impl->get_metadata_path(MD_TYPE_MODULES);
}
#endif
...
static void gpg_import_key(gpgme_ctx_t context, int key_fd, Logger * logger) {
    ...
    if (gpg_err != GPG_ERR_NO_ERROR) {
        auto msg = tfm::format(_("%s: gpgme_data_new_from_fd(): %s"), __func__, gpgme_strerror(gpg_err));
        logger->debug(msg);
    ...
    if (gpg_err != GPG_ERR_NO_ERROR) {
        auto msg = tfm::format(_("%s: gpgme_op_import(): %s"), __func__, gpgme_strerror(gpg_err));
        logger->debug(msg);
        throw LrException(LRE_GPGERROR, msg);
    }
}
...
static std::vector<Key> rawkey2infos(int fd, Logger * logger) {
    ...
    if (gpg_err != GPG_ERR_NO_ERROR) {
        auto msg = tfm::format(_("%s: gpgme_ctx_set_engine_info(): %s"), __func__, gpgme_strerror(gpg_err));
        logger->debug(msg);
        throw LrException(LRE_GPGERROR, msg);
    }
    ...
    if (gpg_err_code(gpg_err) != GPG_ERR_EOF) {
        ...
        auto msg = tfm::format(_("can not list keys: %s"), gpgme_strerror(gpg_err));
        logger->debug(msg);
        throw LrException(LRE_GPGERROR, msg);
    }
...
std::vector<Key> Repo::Impl::retrieve(const std::string & url) {
    ...
    if (fd == -1) {
        auto msg = tfm::format(
            "Error creating temporary file \"%s\": %s", tmp_key_file, std::system_category().message(errno));
        logger->debug(msg);
        throw LrException(LRE_GPGERROR, msg);
    }
    unlink(tmp_key_file);
...
static void ensure_socket_dir_exists(Logger * logger) {
    ...
    if (res != 0 && errno != EEXIST) {
        logger->debug(tfm::format("Failed to create directory \"%s\": %d - %s", dirname, errno, strerror(errno)));
    }
}
    ...
    for (const auto & gpgkeyUrl : conf->gpgkey().get_value()) {
        ...
        for (auto & key_info : key_infos) {
            if (std::find(known_keys.begin(), known_keys.end(), key_info.get_id()) != known_keys.end()) {
                logger->debug(tfm::format(_("repo %s: 0x%s already imported"), id, key_info.get_id()));
                continue;
            }
            ...
            logger->debug(tfm::format(_("repo %s: imported key 0x%s."), id, key_info.get_id()));
        }
    }
}
...
bool Repo::Impl::load_cache(bool throw_except) {
...
bool Repo::Impl::is_metalink_in_sync() {
    ...
    for (auto & hash : hashes) {
        ...
        if (strcmp(chksumHex, hash.lr_metalink_hash->value) != 0) {
            logger->debug(
                tfm::format(_("reviving: failed for '%s', mismatched %s sum."), id, hash.lr_metalink_hash->type));
            return false;
        }
    }
...
bool Repo::Impl::is_repomd_in_sync() {
    ...
    auto same = have_files_same_content(repomd_fn.c_str(), yum_repo->repomd);
    if (same)
...
void Repo::Impl::fetch(const std::string & destdir, std::unique_ptr<LrHandle> && h) {
    ...
    if (g_mkdir_with_parents(destdir.c_str(), 0755) == -1) {
        ...
        throw std::runtime_error(
            tfm::format(_("Cannot create repo destination directory \"%s\": %s"), destdir, err_txt));
    }
    auto tmpdir = destdir + "/tmpdir.XXXXXX";
    if (!mkdtemp(&tmpdir.front())) {
        const char * err_txt = strerror(errno);
        throw std::runtime_error(
            tfm::format(_("Cannot create repo temporary directory \"%s\": %s"), tmpdir.c_str(), err_txt));
    }
    std::unique_ptr<char, std::function<void(char *)>> tmp_dir_remover{
    ...
    if (g_mkdir_with_parents(repodir.c_str(), 0755) == -1) {
        const char * err_txt = strerror(errno);
        throw std::runtime_error(tfm::format(_("Cannot create directory \"%s\": %s"), repodir, err_txt));
    }
    ...
    if (auto * dir = opendir(tmpdir.c_str())) {
        ...
        while (auto ent = readdir(dir)) {
            ...
            try {
                ...
                std::string err_txt = tfm::format(
                    _("Cannot rename directory \"%s\" to \"%s\": %s"), tempElement, target_element, ex.what());
...
bool Repo::Impl::load() {
    auto logger = base->get_logger();
    try {
        if (!get_metadata_path(MD_TYPE_PRIMARY).empty() || load_cache(false)) {
...
std::string Repo::Impl::get_persistdir() const {
    ...
    if (g_mkdir_with_parents(result.c_str(), 0755) == -1) {
        const char * err_txt = strerror(errno);
        throw std::runtime_error(tfm::format(_("Cannot create persistdir \"%s\": %s"), result, err_txt));
    }
    return result;
}
...
int PackageTargetCB::end(TransferStatus /*status*/, const char * /*msg*/) {
    return 0;
}
int PackageTargetCB::progress(double /*totalToDownload*/, double /*downloaded*/) {
    return 0;
}
int PackageTargetCB::mirror_failure(const char * /*msg*/, const char * /*url*/) {
    return 0;
}
...
long LibrepoLog::add_handler(const std::string & file_path, bool debug) {
    ...
    if (!fd) {
        throw std::runtime_error(tfm::format(_("Cannot open %s: %s"), file_path, g_strerror(errno)));
    }
