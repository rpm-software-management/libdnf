...
#include "libdnf/rpm/repo/config_repo.hpp"

#include "../../conf/config-private.hpp"
...
class ConfigRepo::Impl {
    ...
    OptionNumber<std::int32_t> cost{DEFAULT_REPO_COST};
    OptionNumber<std::int32_t> priority{DEFAULT_REPO_PRIORITY};
...
ConfigRepo::ConfigRepo(ConfigRepo && src) noexcept : p_impl(std::move(src.p_impl)) {}
