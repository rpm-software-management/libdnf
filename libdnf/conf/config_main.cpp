...
static int str_to_bytes(const std::string & str) {
    ...
    if (idx < str.length()) {
        if (idx < str.length() - 1) {
            throw Option::InvalidValue(fmt::format("could not convert '%s' to bytes", str));
        }
        switch (str.back()) {
            case 'k':
            case 'K':
...
class ConfigMain::Impl {
    ...
    OptionString user_agent{"libdnf"};  // TODO: getUserAgent()
    ...
    OptionNumber<float> throttle{0, 0, [](const std::string & value) {
...
ConfigMain::Impl::Impl(Config & owner) : owner(owner) {
    ...
    owner.opt_binds().add(
        "tsflags",
        tsflags,
        [&](Option::Priority priority, const std::string & value) { optionTListAppend(tsflags, priority, value); },
        nullptr,
        true);
    ...
    owner.opt_binds().add(
        "excludepkgs",
        excludepkgs,
        [&](Option::Priority priority, const std::string & value) { optionTListAppend(excludepkgs, priority, value); },
        nullptr,
        true);
    owner.opt_binds().add(
        "exclude",
        excludepkgs,  //compatibility with yum
        [&](Option::Priority priority, const std::string & value) { optionTListAppend(excludepkgs, priority, value); },
        nullptr,
        true);
    ...
    owner.opt_binds().add(
        "includepkgs",
        includepkgs,
        [&](Option::Priority priority, const std::string & value) { optionTListAppend(includepkgs, priority, value); },
        nullptr,
        true);
...
OptionBool & ConfigMain::allow_vendor_change() { return p_impl->allow_vendor_change; }
