...
struct _DnfStateClass
{
    ...
    void (*percentage_changed)(DnfState *state, guint value);
    void (*allow_cancel_changed)(DnfState *state, gboolean allow_cancel);
    void (*action_changed)(DnfState *state, DnfStateAction action, const gchar *action_hint);
...
#define dnf_state_set_number_steps(state, steps)                                                   \
    dnf_state_set_number_steps_real(state, steps, G_STRLOC)
#define dnf_state_set_steps(state, error, value, args...)                                          \
    dnf_state_set_steps_real(state, error, G_STRLOC, value, ##args)

typedef gboolean (*DnfStateErrorHandlerCb)(const GError *error, gpointer user_data);
...
gboolean
dnf_state_check(DnfState *state, GError **error) G_GNUC_WARN_UNUSED_RESULT;
gboolean
dnf_state_done_real(DnfState *state, GError **error, const gchar *strloc) G_GNUC_WARN_UNUSED_RESULT;
gboolean
dnf_state_finished_real(DnfState *state,
                        GError **error,
                        const gchar *strloc) G_GNUC_WARN_UNUSED_RESULT;
gboolean
