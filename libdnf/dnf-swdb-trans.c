...
static gint
_trans_compare (gconstpointer a, gconstpointer b)
{
 ...
 **/
void
dnf_swdb_trans_merge (DnfSwdbTrans *self, DnfSwdbTrans *other)
{
    if (!self || !other) {
        return;
    }
    ...
    if (other->merged_tids) {
        g_array_append_vals (merged, other->merged_tids->data, other->merged_tids->len);
    } else {
...
static void
_pkg_pair_set (_pkg_pair *p, DnfSwdbPkg *pkg2)
{
    if (p->second) {
 ...
 **/
static void
_merge_altered_packages (_pkg_pair *prev, DnfSwdbPkg *pkg)
{
 ...
 **/
static void
_merge_handle_erase (_pkg_pair *prev, DnfSwdbPkg *pkg)
{
    DnfSwdbPkg *pkg1 = prev->first;

    // new package is being installed
    if (!g_strcmp0 (pkg->state, "Install") || !g_strcmp0 (pkg->state, "True-Install") ||
 ...
 **/
static void
_merge_handle_install (GHashTable *pkgs, _pkg_pair *prev, DnfSwdbPkg *pkg)
{
    DnfSwdbPkg *pkg1 = prev->first;

    // handle Erase/Obsoleted
    if (!g_strcmp0 (pkg->state, "Erase") || !g_strcmp0 (pkg->state, "Obsoleted")) {
    ...
    else if (!g_strcmp0 (pkg->state, "Update") || !g_strcmp0 (pkg->state, "Downgrade")) {
 ...
 **/
static void
_merge_handle_alter (_pkg_pair *prev, DnfSwdbPkg *pkg)
{
    DnfSwdbPkg *pkg1 = prev->first;
    ...
    // handle Downgraded/Updated
    else if (!g_strcmp0 (pkg->state, "Downgraded") || !g_strcmp0 (pkg->state, "Updated")) {
        /*
         ...
         */
        if (!prev->second) {
            if (!g_strcmp0 (pkg1->state, "Downgrade") || !g_strcmp0 (pkg1->state, "Update")) {
    ...
    else if (!g_strcmp0 (pkg->state, "Downgrade") || !g_strcmp0 (pkg->state, "Update")) {
        /*
...
_merge_package (GHashTable *pkgs, DnfSwdbPkg *pkg)
{
     ...
     */
    if (!g_strcmp0 (pkg1->state, "Erase") || !g_strcmp0 (pkg1->state, "Obsoleted")) {
     ...
     * Update/Downgrade - move install to new package
     */
    else if (!g_strcmp0 (pkg1->state, "Install") || !g_strcmp0 (pkg1->state, "True-Install") ||
     ...
     */
    else if (!g_strcmp0 (pkg1->state, "Downgrade") || !g_strcmp0 (pkg1->state, "Update") ||
             !g_strcmp0 (pkg1->state, "Obsoleting") || !g_strcmp0 (pkg1->state, "Downgraded") ||
 ...
 **/
static GPtrArray *
_trans_merged_output (DnfSwdbTrans *self, const gchar *type)
{
    DnfSwdb *swdb = self->swdb;
    ...
    return _load_output (swdb->db, self->tid, dnf_swdb_get_output_type (swdb, type));
}
...
dnf_swdb_trans_data (DnfSwdbTrans *self)
{
    ...
    while (sqlite3_step (res) == SQLITE_ROW) {
        gchar *tmp_reason = _get_description (db, sqlite3_column_int (res, 6), S_REASON_TYPE_BY_ID);

        gchar *tmp_state = _get_description (db, sqlite3_column_int (res, 7), S_STATE_TYPE_BY_ID);
 ...
 **/
gboolean
dnf_swdb_trans___lt__ (DnfSwdbTrans *first, DnfSwdbTrans *second)
{
 ...
 **/
gboolean
dnf_swdb_trans___gt__ (DnfSwdbTrans *first, DnfSwdbTrans *second)
{
    return dnf_swdb_trans_compare (first, second) > 0;
}
