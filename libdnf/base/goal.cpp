...
private:
    ...
    std::vector<std::tuple<GoalAction, std::string, GroupJobSettings>> group_specs;
...
void Goal::add_group_install(const std::string & spec, const GroupJobSettings & settings) {
    ...
    for (auto [action, ids, settings] : rpm_ids) {
        switch (action) {
            case GoalAction::INSTALL: {
                ...
                rpm_goal.add_user_installed(ids);
            } break;
            case GoalAction::INSTALL_OR_REINSTALL:
    ...
    for (auto group : group_query) {
        ...
        for (const auto & pkg : packages) {
            ...
            if (pkg_condition.empty()) {
                auto [pkg_problem, pkg_queue] = add_install_to_goal(transaction, pkg_name, pkg_settings);
                rpm_goal.add_group_installed(pkg_queue);
                packages_res |= pkg_problem;
            } else {
                ...
                if (!query.empty()) {
                    libdnf::solv::IdQueue idq;
                    for (const auto & pkg : query) {
                        idq.push_back(pkg.get_id().id);
                    }
                    rpm_goal.add_group_installed(idq);
                }
            }
        }
