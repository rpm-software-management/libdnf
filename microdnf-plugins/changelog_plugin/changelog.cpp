...
ChangelogCommand::ChangelogCommand(Command & parent) : Command(parent, "changelog") {
    ...
    cmd.set_short_description("Show changelog data of packages");
    ...
    since->set_short_description(
        "show changelog entries since DATE. To avoid ambiguosity, YYYY-MM-DD format is required.");
    since->set_has_value(true);
    since->link_value(since_option);

    auto count = parser.add_new_named_arg("count");
    count->set_long_name("count");
    count->set_short_description("show given number of changelog entries per package");
    ...
    upgrades->set_short_description(
        "show only new changelog entries for packages, that provide an upgrade for some of already installed "
        "packages.");
    ...
    keys->set_short_description("List of packages specifications");
