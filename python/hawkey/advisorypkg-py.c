...
typedef struct
{
    PyObject_HEAD DnfAdvisoryPkg *advisorypkg;
} _AdvisoryPkgObject;
...
advisorypkg_richcompare(PyObject *self, PyObject *other, int op)
{
    ...
    if (!advisorypkg_converter(self, &cself) || !advisorypkg_converter(other, &cother)) {
...
PyTypeObject advisorypkg_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "_hawkey.AdvisoryPkg", /*tp_name*/
