...
sdbus::MethodReply RepoConf::get(sdbus::MethodCall & call) {
    ...
    if (lst.empty()) {
        throw sdbus::Error(dnf5daemon::ERROR_REPOCONF, "Repository not found");
    } else if (lst.size() > 1) {
        throw sdbus::Error(dnf5daemon::ERROR_REPOCONF, "Multiple repositories found");
    } else {
        auto reply = call.createReply();
        reply << lst[0];
        return reply;
...
std::vector<std::string> RepoConf::enable_disable_repos(const std::vector<std::string> & ids, const bool enable) {
    ...
    for (auto & config_file : changed_config_files) {
        try {
            ...
            throw sdbus::Error(
                dnf5daemon::ERROR_REPOCONF, std::string("Unable to write configuration file: ") + e.what());
        }
    }
...
sdbus::MethodReply RepoConf::enable_disable(sdbus::MethodCall && call, const bool & enable) {
    ...
    call >> ids;
    auto is_authorized = session.check_authorization(dnf5daemon::POLKIT_REPOCONF_WRITE, sender);
    if (!is_authorized) {
        throw sdbus::Error(dnf5daemon::ERROR_REPOCONF, "Not authorized.");
    }
    auto reply = call.createReply();
